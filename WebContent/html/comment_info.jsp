...
<html>
  ...
  String sql="";
  sql="select * from comment,user where comment_id='"+comment_id+"'and author_id=user_id ";
  ResultSet rs=null;
  rs=util.query(sql);
  if(rs.next())
  %>
                      ...
                      <div class="p_content  "> 
                        ...
                        <div class="user-hide-post-down" style="display: none;">
                         ...
                         <div class="core_reply_tail clearfix">
                          <a class="p_reply p_reply_first" href="#" onclick="reply()" rel="noopener">回复</a>
                          <%
                          String user_id=session.getAttribute("user_id").toString();
                          sql="select 1 from star where comment_id='"+comment_id+"'and user_id='"+user_id+"' ";
                          ResultSet rs2=util.query(sql);
                          if(rs2.next()){
                          %>
                          <a class="p_reply p_reply_first" href="#" rel="noopener">已点赞</a>
                          <%
                          }else{
                          %> 
                          <a class="p_reply p_reply_first" href="dostar.jsp?comment_id=<%=comment_id %>" rel="noopener">点赞</a>
						 <%} %>
...
<%
sql="select * from reply,user where comment_id='"+comment_id+"' and reply.user_id=user.user_id ";
rs=util.query(sql);
int i=1;
while(rs.next()){
%>                
  ...
  <div class="d_post_content_main">
    <div class="p_content  p_content p_content_icon_row1 p_content_nameplate">
         <div class="d_post_content j_d_post_content  clearfix" id="post_content_136437347937" style="display:;">
  			<h3><%=rs.getString("reply_content") %></h3>
         </div>
        <br>
       <br>
      <div class="user-hide-post-down" style="display: none;">
    ...
    <div class="core_reply j_lzl_wrapper">
      <div class="core_reply_tail ">
         <ul class="p_tail">
          ...
          <li>
            <span><%=rs.getString("reply_time") %></span>
          </li>
        </ul> 
      </div>
   ...
   function reply(){
	   layui.use('layer', function(){
		   ...
		   layer.prompt({title: '回复', formType: 2}, function(text, index){
			    layer.close(index);
  			    window.location.href=encodeURI("doreply.jsp?reply_content="+text+"&comment_id="+comment_id);
 			  });
		 });      
	   }
   </script>
